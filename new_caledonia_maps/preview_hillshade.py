...
preview_svg_white_filename = preview_filepath + '%s_white_%.2f.svg'
preview_svg_black_filename = preview_filepath + '%s_black_%.2f.svg'
...
for threshold_delta in [0.05, 0.10, 0.15]:
    ...
    docker_run(
        'potrace %s -o %s -b svg -k %f -i' % (
            ...
            preview_svg_white_filename % ('dark', threshold),
            threshold
        )
    )
    docker_run(
        'cp %s %s' % (
                preview_svg_white_filename % ('dark', threshold),
                preview_svg_white_filename % ('light', threshold)
        )
    )
...
for threshold_delta in [0.05, 0.10, 0.15, 0.25, 0.40]:
    ...
    docker_run(
        'potrace %s -o %s -b svg -k %f' % (
            preview_pnm_filename,
            preview_svg_black_filename % ('dark', threshold),
            threshold
        )
    )
    docker_run(
        'cp %s %s' % (
            preview_svg_black_filename % ('dark', threshold),
            preview_svg_black_filename % ('light', threshold)
        )
    )
